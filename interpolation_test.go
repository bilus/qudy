...
func TestCompileReadsEachInterpolation(t *testing.T) {
	...
	for _, c := range []testCase{
		...
		{"a name path", "x := ~p.Type.Text", "g.generate(`x := %s\n`, p.Type.Text)"},
		...
		{"an index of the scaffolding needs braces", "~{fns[0]}(a) = x", "g.generate(`%s(a) = x\n`, fns[0])"},
		{"a generic parameter stays text", `func F~name[R any](\`, "g.generate(`func F%s[R any](`, name)"},
		{"a fresh name", "if errName# != nil {", "g.generate(`if %s != nil {\n`, errName)"},
		{"two fresh names", "local#, errName# := f()", "g.generate(`%s, %s := f()\n`, local, errName)"},
		...
		{"a tilde comment goes, with the spaces before it", "x := ~n   ~// why", "g.generate(`x := %s\n`, n)"},
		{"a comment without a tilde stays", "// DispatchTable maps names", "g.generate(`// DispatchTable maps names\n`)"},
		{"a tilde comment on its own empties the line", "~// a note", "g.generate(`\n`)"},
		{"a percent doubles", "50% of ~n", "g.generate(`50%% of %s\n`, n)"},
		{"a brace inside a string does not close the braces", `~{q("a}b")} x`, "g.generate(`%s x\n`, q(\"a}b\"))"},
	} {
...
func TestCompileRejectsAnInterpolation(t *testing.T) {
	...
	for _, c := range []testCase{
		...
		{"whose braces do not close", "x := ~{a + b", "does not close"},
		{"at the end of a line", "x := ~", "starts with a name"},
		{"a hash that follows nothing", "x := #n", "follows one"},
	} {
		t.Run(c.description, func(t *testing.T) {
			_, err := qudy.Compile("test.go", []byte(template("//`"+c.line)), "g.generate", qudy.NewScope("sc", "newScope()", "fresh"))
